...
const ClientManager = () => {
  ...
  const generateClientCode = (existingClients) => {
    const max = existingClients.reduce((acc, c) => {
      const match = c.clientCode && c.clientCode.match(/^CLI(\d{3})$/);
      if (match) {
  ...
  // Update filteredClients state when the memoized value changes
  useEffect(() => {
  ...
  const handleOpen = (client, idx) => {
    if (client) {
      setForm({ ...client });
      setOriginalClientId(client.id || null);
  ...
  const handleDateChange = (date, field) => {
    setForm({ ...form, [field]: date ? new Date(date).toISOString().slice(0, 10) : '' });
  };
  ...
  const isCurrentStepCompleted = () => {
    switch (activeStep) {
      case 0: // Basic Information
        return form.clientName && form.clientName.trim() !== '' && 
               form.clientCode && form.clientCode.trim() !== '' &&
               form.businessType && form.businessType.trim() !== '';
      ...
      case 3: // Contact Management
        return form.contacts && form.contacts.length > 0 && 
               form.contacts.some(contact => 
                 contact.name && contact.name.trim() !== '' &&
                 contact.number && contact.number.trim() !== ''
               );
      case 4: // Business Terms
        return form.paymentTerms && form.paymentTerms.trim() !== '' && 
               form.creditLimit && form.creditLimit.trim() !== '';
      case 5: // Products & Services
        return form.products && form.products.length > 0 && 
  ...
  const removeContact = (idx) => setForm({ 
    ...form, 
    contacts: form.contacts.filter((_, i) => i !== idx) 
  });

  const setPrimaryContact = (idx) => {
    const updated = form.contacts.map((contact, i) => ({
      ...contact,
      isPrimary: i === idx
    }));
    setForm({ ...form, contacts: updated });
    ...
    <Box sx={{ 
      ...
      >
        ...
        <Box sx={{ position: 'relative', zIndex: 1 }}>
          ...
          >
                  Comprehensive client information management with advanced features
          </Typography>
              </Box>
            </Box>
        ...
        <CardContent sx={{ p: 4 }}>
          ...
          <Box sx={{ 
            ...
            <Box>
              ...
              >
                  Manage your client relationships with comprehensive information tracking
              </Typography>
            </Box>
        ...
        <DialogActions sx={{ 
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            ...
            {activeStep < steps.length - 1 ? (
              ...
              >
                {loading ? 'Saving...' : (editIndex !== null ? 'Update Client' : 'Create Client')}
              </Button>
            )}
          </Box>
